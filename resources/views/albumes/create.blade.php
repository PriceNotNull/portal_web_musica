...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10 bg-white dark:bg-gray-800 rounded-lg shadow">
    <h1 class="text-3xl font-bold mb-8 text-gray-900 dark:text-gray-100">Crear Álbum</h1>
    ...
    <form action="{{ route('albumes.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        ...
        <div>
            ...
            <select name="artista_id" id="artista_id" required
                ...
                @foreach ($artistas as $artista)
                    <option value="{{ $artista->id }}" @if(old('artista_id') == $artista->id) selected @endif>{{ $artista->nombre }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div>
            ...
            <input type="number" name="año" id="año" value="{{ old('año') }}" required
                class="mt-1 block w-full rounded-md border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
        </div>
